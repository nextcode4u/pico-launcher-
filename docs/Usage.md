...
- `romBrowserHideFolders` - Hides hidden/system folders (`.` / `_` prefixes and FAT hidden) by default. This now defaults to `true`.
- `showHiddenItemsToggle` - Optional. Set to `true` if you want the “Hidden Items” toggle to appear in the in-app Display Settings menu. If omitted or `false`, hidden items stay hidden and the toggle is not shown.
...
Example fragment to enable the toggle in `/_pico/settings.json`:

```json
{
  "romBrowserHideFolders": true,
  "showHiddenItemsToggle": true
}
```
